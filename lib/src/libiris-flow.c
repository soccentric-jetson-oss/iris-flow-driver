...
struct iris_flow_handle *iris_flow_open(const char *device_path) {
	if (!device_path) { errno = EINVAL; return NULL; }
...
int iris_flow_start_stream(struct iris_flow_handle *h, uint32_t width, uint32_t height, uint32_t format, uint32_t fps) {
	if (!h) return -EINVAL;
...
int iris_flow_get_frame(struct iris_flow_handle *h, uint32_t *frame_id, uint64_t *timestamp_ns) {
	...
	if (ret < 0) ret = -errno;
	else { if (frame_id) *frame_id = info[0]; if (timestamp_ns) *timestamp_ns = info[3]; }
	pthread_mutex_unlock(&h->lock);
	return ret;
}

int iris_flow_set_isp(struct iris_flow_handle *h, int32_t brightness, int32_t contrast, int32_t saturation) {
	if (!h) return -EINVAL;
